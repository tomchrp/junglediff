...
MODIFICATIONS :
- Retrait de la colonne `position` dans la table `match_participants` pour s'aligner
  sur la nouvelle topographie Match-V5 définie dans models.py.
- Mise à jour de l'index `ix_participant_filter` pour ne plus dépendre de la 
  colonne obsolète `position`.
"""
...
def upgrade() -> None:
    """
    Applique la migration initiale sur la base de données vierge.
    Instancie les tables matches, players, match_participants et match_timelines.
    La structure a été corrigée pour éliminer la contrainte bloquante sur 'position'.
    """
    ...
    op.create_index('ix_participant_filter', 'match_participants', ['puuid', 'champion_id'], unique=False)
...
def downgrade() -> None:
    """
    Exécutée lors du rollback de la migration.
    Assure la suppression propre de toutes les tables créées initialement.
    """
    # ### commands auto generated by Alembic - please adjust! ###

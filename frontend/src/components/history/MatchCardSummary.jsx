/**
 * ============================================================================
 * FICHIER : frontend/src/components/history/MatchCardSummary.jsx
 * PROJET  : JungleDiff
 *
 * DESCRIPTION :
 * Sous-composant d'affichage. Génère le tableau de bord global de la partie
 * en listant les 10 joueurs répartis en deux colonnes (Équipe 100 et 200).
 * * DESIGN SYSTEM : Utilisation des surfaces neutres (surface-solid, surface-elevated)
 * et accentuation de la cible via les jetons lol-gold. Élargissement de la 
 * zone cliquable (group) pour une meilleure UX.
 * ============================================================================
 */

import React from 'react';

const MatchCardSummary = ({ team100, team200, playerPuuid, versionDDragon, championMap, currentServer, onPlayerSearch }) => {

    /**
     * Construit la ligne visuelle d'un joueur avec son KDA, son champion et son inventaire.
     * Isole l'événement de clic pour la redirection de profil.
     */
    const renderParticipantRow = (p) => {
        const isTargetUser = p.puuid === playerPuuid;
        const playerItems = [p.item0, p.item1, p.item2, p.item3, p.item4, p.item5, p.item6];
        const imageChampName = championMap[p.championId] || "Inconnu";

        return (
            <div key={p.puuid} className={`flex items-center justify-between p-2 rounded-md transition-colors ${isTargetUser ? 'bg-lol-gold/10 border border-lol-gold/30 shadow-sm' : 'hover:bg-surface-elevated border border-transparent'}`}>

                {/* L'événement onClick est remonté sur le div parent pour agrandir la zone cliquable */}
                <div
                    className="flex items-center gap-2 w-5/12 min-w-0 shrink-0 cursor-pointer group"
                    onClick={(e) => { e.stopPropagation(); onPlayerSearch(currentServer, p.riotIdGameName, p.riotIdTagline); }}
                >
                    <img
                        src={`https://ddragon.leagueoflegends.com/cdn/${versionDDragon}/img/champion/${imageChampName}.png`}
                        alt={imageChampName}
                        className={`w-6 h-6 rounded border shrink-0 ${isTargetUser ? 'border-lol-gold' : 'border-border-strong'}`}
                        onError={(e) => e.target.src = 'https://ddragon.leagueoflegends.com/cdn/14.12.1/img/profileicon/29.png'}
                    />
                    {p.teamPosition && (
                        <img
                            src={`/assets/lanes/${p.teamPosition.toLowerCase()}.png`}
                            alt={p.teamPosition}
                            className="w-4 h-4 shrink-0 object-contain"
                            onError={(e) => { e.target.style.display = 'none'; }}
                        />
                    )}
                    <span className={`text-xs truncate transition-colors group-hover:underline ${isTargetUser ? 'text-lol-gold font-bold hover:text-lol-goldHover' : 'text-lol-textMuted group-hover:text-gray-100'}`}>
                        {p.riotIdGameName}
                    </span>
                </div>

                <div className="text-center text-[11px] text-lol-textMuted font-medium w-3/12 shrink-0">
                    {p.kills}/{p.deaths}/{p.assists}
                </div>

                <div className="flex gap-0.5 w-4/12 justify-end shrink-0">
                    {playerItems.map((itemId, idx) => (
                        <div key={idx} className="w-5 h-5 bg-surface-solid rounded-[3px] border border-border-glass overflow-hidden shrink-0">
                            {itemId > 0 && (
                                <img
                                    src={`https://ddragon.leagueoflegends.com/cdn/${versionDDragon}/img/item/${itemId}.png`}
                                    alt="Objet"
                                    className="w-full h-full object-cover"
                                    onError={(e) => e.target.style.display = 'none'}
                                />
                            )}
                        </div>
                    ))}
                </div>
            </div>
        );
    };

    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-2">
            <div className="flex flex-col gap-1 border-r lg:border-r border-border-glass pr-2">
                {team100.map(p => renderParticipantRow(p))}
            </div>
            <div className="flex flex-col gap-1 pl-1">
                {team200.map(p => renderParticipantRow(p))}
            </div>
        </div>
    );
};

export default MatchCardSummary;